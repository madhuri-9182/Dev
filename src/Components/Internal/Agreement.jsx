            ...
            {data.slice(0, showAll ? data.length : data.length).map((item, index) => (
              <div key={index} className='font-semibold'>
                ...
                >
                  ...
                  <div className="px-3 py-[10px] w-auto text-black font-semibold  ">
                    <label htmlFor="">0-4 Years</label>
                    <input
                      type="number"
                      disabled={editClientUser !== index}
                      value={item.salaries['0-4 Years'].toFixed(2)}
                      className={`block w-full text-left font-normal pt-[4px] ${editClientUser === index
                        ? ' focus:outline-none border border-[#E8DEF8] focus:ring-1 focus:ring-blue-500 rounded-lg h-[30px] pl-[10px]'
                        : 'border border-none'
                        }`}
                    />
                  </div>
