...
<div className="p-2 px-16">
  <div>
    <h2 className="text-base font-semibold text-black mb-1">Messages:</h2>
  </div>
  <div className="flex flex-col h-[500px] p-4 bg-gradient-to-r from-blue-500 to-blue-300 rounded-lg shadow-lg">
    <div className="flex-1 overflow-y-auto bg-transparent text-white p-3 rounded-lg border border-transparent">
      {messages.map((message) => (
        <p key={message.id} className="mb-1 text-sm">{message.text}</p>
      ))}
    </div>
  </div>
  <div className="mt-3">
    <label htmlFor="newMessage" className="text-black text-sm font-medium">Type Here:</label>
    <div className="flex items-center mt-2 rounded-lg px-3 py-1 bg-[#E7E4E8]">
      <input
        id="newMessage"
        type="text"
        value={newMessage}
...
export  {Message as InternalMessages};
