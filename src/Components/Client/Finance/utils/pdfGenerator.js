...
/**
 * Creates table data from finance items
 ...
 */
const addTableToPdf = (doc, tableData) => {
  autoTable(doc, {
    startY: 50,
    head: [
 ...
 * @param {string} logoPath - Path to company logo
 */
export const generateCurrentMonthPdf = async (
  data,
  totalAmount,
  logoPath
) => {
 ...
 * @param {string} logoPath - Path to company logo
 */
export const generateLastMonthPdf = async (
  data,
  totalAmount,
  logoPath
) => {
  try {
    ...
    // Save the PDF
    doc.save(`${lastMonthName}_finance_report.pdf`);
    return true;
  } catch (error) {
    console.error("Error generating PDF:", error);
    return false;
